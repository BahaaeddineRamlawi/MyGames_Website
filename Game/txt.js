export class Txt {
    ...
    draw(context){
        ...
        context.fillText('Time: ' + ((this.game.time - this.game.differencetime) * 0.001).toFixed(1), 20, 80);
    ...
    myinput(){
        if (confirm("Save This Run")){
            ...
            if (name == null || name == "") {
                ...
                let t = `${name}=${this.game.score}=${((this.game.time - this.game.differencetime) * 0.001).toFixed(1)}`;
                this.goToPage2(t)
            }
        }
    }
