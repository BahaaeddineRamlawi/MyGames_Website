...
export class Running extends State {
  ...
  HandleInput(input) {
    this.game.particles.unshift(
      new Dust(
        this.game,
        this.game.player.x + this.game.player.truewidth * 0.6,
        this.game.player.y + this.game.player.trueheight
      )
    );
    if (input.includes("ArrowUp") || input.includes("w")) {
...
export class MovingBack extends State {
  ...
  HandleInput(input) {
    if (input.includes("ArrowUp") || input.includes("w")) {
      ...
      (input.includes("ArrowDown") || input.includes("s")) &&
      !(
        input.includes("ArrowRight") ||
        input.includes("d") ||
        input.includes("ArrowLeft") ||
    ...
    if (this.counter >= 30 && this.counter <= 85) {
      this.game.particles.unshift(
        new Rings(
          this.game,
          this.game.player.x +
            this.game.player.truewidth -
            (this.game.player.xafter - this.game.player.xbefore),
          this.game.player.y + this.game.player.trueheight * 0.5
        )
      );
      this.game.player.xbefore = 20;
      this.game.player.xafter = -50;
...
export class Hit extends State {
  ...
  HandleInput(input) {
    ...
    if (this.game.player.frameX >= 20 && this.game.player.checkonGround()) {
      ...
      !this.game.player.checkonGround()
    ) {
      this.game.player.setState(states.Falling, 1);
    }
  }
    ...
    if (this.game.player.checkonGround()) {
      for (let i = 0; i < 5; ++i) {
        ...
        this.game.particles.unshift(
          new CrushDust(
            this.game,
            this.game.player.x + this.game.player.truewidth * 0.5,
            this.game.player.y + this.game.player.trueheight
          )
        );
      }
