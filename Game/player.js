...
export class Player {
  constructor(game) {
    ...
    this.states = [
      ...
      new Moving(this.game),
      new Jumping(this.game),
      new Falling(this.game),
      new Running(this.game),
      new MovingBack(this.game),
      new Punching(this.game),
      new Upunching(this.game),
      new Hit(this.game),
      new Crushing(this.game),
      new Dying(this.game),
    ];
  }
  ...
  draw(context) {
    ...
    if (this.game.hitbox)
      context.strokeRect(
        this.x + this.xbefore,
        this.y + this.ybefore,
        this.truewidth - this.xafter,
        this.trueheight - this.yafter
      );
    context.drawImage(
      this.image,
  ...
  checkCollision() {
    this.game.enemies.forEach((mob) => {
      if (
        mob.x < this.x + this.truewidth - (this.xafter - this.xbefore) &&
        ...
        this.game.collisions.push(
          new MobsCollision(
            this.game,
            mob.x + mob.truewidth * 0.5,
            mob.y + mob.trueheight * 0.5
          )
        );
        if (
          this.currentstate.state == "CRUSHING" &&
          mob.getType() == "GroundMob1"
        )
          this.game.score += 4;
        else if (
          this.currentstate.state == "CRUSHING" &&
          mob.getType() == "GroundMob2"
        )
          this.game.score += 5;
        else if (
          this.currentstate.state == "PUNCHING" &&
          mob.getType() == "GroundMob1"
        )
          this.game.score += 2;
        else if (
          ...
          this.currentstate.state == "UPUNCHING" &&
          mob.getType() == "FlyingMob"
        )
          this.game.score++;
        else if (
          this.currentstate.state == "UPUNCHING" &&
